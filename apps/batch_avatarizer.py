...
def discover_econ_names(econ_results_path):
    """Discover all available ECON result names from the obj directory."""
    obj_dir = osp.join(econ_results_path, "obj")
    
    if not osp.exists(obj_dir):
        print(f"Error: obj directory not found: {obj_dir}")
        return []
    
    # Find all *_smpl_00.npy files to get the base names
    smpl_files = glob.glob(osp.join(obj_dir, "*_smpl_00.npy"))
    
    names = []
    for smpl_file in smpl_files:
        # Extract base name by removing _smpl_00.npy suffix
        basename = osp.basename(smpl_file)
        name = basename.replace("_smpl_00.npy", "")
        names.append(name)
    
    return names

def process_single_result(econ_results_path, name, gpu_id, use_uv=False, use_dress=False):
    """Process a single ECON result"""
    ...
    # Set up paths for ECON results structure
    obj_dir = osp.join(econ_results_path, "obj")
    png_dir = osp.join(econ_results_path, "png")
    
    # Loading SMPL-X and econ objs inferred with ECON
    prefix = osp.join(obj_dir, name)
    ...
    if not osp.exists(econ_path):
        print(f"Warning: ECON file not found: {econ_path}")
        return False
    ...
    if not osp.exists(f"{prefix}/econ_icp_rgb.ply"):
        masked_image = osp.join(png_dir, f"{name}_cloth.png")
        if not osp.exists(masked_image):
    ...
    if use_uv:
        ...
        original_pose = smpl_out_lst[3].full_pose
...
def main():
    parser = argparse.ArgumentParser(description="Batch process ECON results from a folder")
    parser.add_argument("-p", "--path", type=str, required=True, 
                       help="Path to ECON results folder (should contain obj/ and png/ subdirectories)")
    parser.add_argument("-g", "--gpu", type=int, default=0, 
                       help="GPU device ID")
    parser.add_argument("-uv", action="store_true", 
                       help="Generate UV textures")
    parser.add_argument("-dress", action="store_true", 
                       help="Use dress mode for torn dress/skirt handling")
    parser.add_argument("--filter", type=str, default="", 
                       help="Filter results by name pattern")
    parser.add_argument("-n", "--name", type=str, default="", 
                       help="Process only a specific name (if provided, -p should point to ECON results folder)")
    ...
    # Handle single name processing (similar to original avatarizer.py)
    if args.name:
        print(f"Processing single result: {args.name}")
        success = process_single_result(args.path, args.name, args.gpu, args.uv, args.dress)
        if success:
    ...
    print(f"Processing ECON results in: {args.path}")
    
    # Discover all available names from the ECON results
    names = discover_econ_names(args.path)
    
    if not names:
        print("No valid ECON result files found!")
        print("Expected structure: <econ_results_path>/obj/ with *_smpl_00.npy files")
        print("                   <econ_results_path>/png/ with *_cloth.png files")
        return
    ...
    if args.filter:
        ...
        if not names:
            print(f"No results found matching filter: {args.filter}")
            return
    
    print(f"Found {len(names)} results to process:")
    for name in names:
        print(f"  - {name}")
    ...
    for name in names:
        try:
            success = process_single_result(args.path, name, args.gpu, args.uv, args.dress)
            if success:
                successful += 1
            else:
                failed += 1
    ...
    if failed > 0:
        print(
            colored(
                "If dress/skirt is torn in any result, delete the result folder and regenerate with -dress flag", 
                "yellow"
            )
        )
